...
export default function DeletePhotoConfirmationDialog({
  open,
  deleting,
  photoLabel,
  onCancel,
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-photo-confirmation-title"
        aria-describedby="delete-photo-confirmation-body"
        className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-4 shadow-xl"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="delete-photo-confirmation-title" className="text-base font-bold text-slate-900">
              Hapus foto ini?
            </h2>
            <p id="delete-photo-confirmation-body" className="mt-2 text-sm text-slate-600">
              Foto {photoLabel ? `"${photoLabel}"` : 'yang dipilih'} akan dihapus dari server. Slot ini harus difoto ulang sebelum laporan dapat dikirim.
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label="Tutup konfirmasi hapus foto"
          >
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-red-600 px-3 text-sm font-semibold text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-200 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
